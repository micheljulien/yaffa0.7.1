...
// beginning of the loop.  Otherwise, discard the current loop control 
// parameters and continue execution immediately following the loop.
void _plus_loop_sys(void) {
  cell_t limit = rPop();    // fetch limit
  cell_t index_before = rPop();    // fetch index
  cell_t index_after = index_before + pop();
  if ((index_before < limit && index_after >= limit) ||
      (index_before >= limit && index_after < limit))  {
    ip++;
    if (rPop() != LOOP_SYS) {
      push(-22);
      _throw();
...
SKIP:                // common code to skip initial character
  ...
  push(accum & 0xFFFF); // Push the resultant number
  push(accum >> 16);
...
void _fm_slash_mod(void) {
  cell_t n1 = pop();
  dcell_t d1 = dcell_t(pop()) << sizeof(cell_t)*8;
  d1 |= (pop() & 0xFFFF);
  push(d1 /  n1);
  push(d1 %  n1);
}
...
void _m_star(void) {
  int32_t temp = (int32_t)pop() * (int32_t)pop();
  push(temp & 0xffff);
  push(temp >> 16);
}
...
void _quit(void) {
  rtos = -1;
  ip = NULL;                  // to get out of executeWord
  cpToIn = cpSourceEnd + 1;   // to stop outer interpreter
  Serial.flush();
}
...
// ( n -- d )
// tested and fixed by Alex Moskovskij
// Modified by Michel Julien to comply with:
...
// ( "ccc<paren>" -- )
// Parse and display ccc delimitied by ) (right parenthesis). .( is an imediate
// word
...
void _dump(void) {
  ...
  while (addr < (uint8_t*)addr_end) {
    ...
    } else {  // if not hex, go decimal
      if (addr < 1000) serial_print_P(zero_str);
      if (addr < 100) serial_print_P(zero_str);
      if (addr < 10) serial_print_P(zero_str);
    }
    Serial.print((size_t)addr, base);
